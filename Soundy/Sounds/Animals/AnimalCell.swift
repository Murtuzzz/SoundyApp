...
class AnimalCollectionCell: UICollectionViewCell {
    ...
    @objc private func audioStateChanged(_ notification: Notification) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // Проверяем только звуки животных и именно нашу ячейку
            if category == "animals" && trackIndex == self.cellIndex {
                if isPlaying {
    ...
    public func configure(label: String, image: UIImage, index: Int) {
        ...
        let isCurrentlyPlaying = AudioManager.shared.isPlaying(trackIndex: index, category: "animals")
    ...
    public func changeCondition(_ num: Int) {
        ...
        AudioManager.shared.toggleTrack(num, trackName: mainLabel.text ?? "", musicList: musicList, category: "animals")
    }
